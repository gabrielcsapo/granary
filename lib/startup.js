...
module.exports = function(log) {
    ...
    Startup.check = function(options) {
        ...
        try {
            var granaryrc = JSON.parse(fs.readFileSync(
                path.resolve(options.directory || process.cwd(), '.granaryrc.json'),
                'utf8'
            ));
            if (granaryrc.url) {
                // .granaryrc options can be overriden by environmental variables
                options.url = options.url || granaryrc.url;
            }
        } catch (ex) {
            log.debug(ex);
        }
    ...
    Startup.granaryRequest = function(url, project, extra, options) {
        ...
        remote.granaryCheck(url, project, extra)
            .then(function(granary) {
                if (granary.available === false || options.install || options.rebuild) {
                    extra.create = true;
                    if (options.rebuild) {
                        extra.force = true;
                    }
